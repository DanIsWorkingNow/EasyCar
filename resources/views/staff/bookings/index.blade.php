...
<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; padding: 2rem 0;">
    <div class="container">
        ...
        <div style="background: rgba(255,255,255,0.95); backdrop-filter: blur(15px); border-radius: 25px; padding: 2.5rem; margin-bottom: 2rem; box-shadow: 0 25px 50px rgba(0,0,0,0.15);">
            ...
            <p style="color: #6b7280; font-size: 1.1rem; margin: 0.5rem 0 0 0;">Approve, reject, and manage bookings for your branch</p>
        </div>
        ...
        <div style="background: rgba(255,255,255,0.95); backdrop-filter: blur(15px); border-radius: 20px; padding: 2rem; margin-bottom: 2rem; box-shadow: 0 20px 40px rgba(0,0,0,0.1);">
            <form method="GET" action="{{ route('staff.bookings.index') }}">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-3">
                        <div style="display: flex; gap: 0.5rem;">
                            <button type="submit" style="background: linear-gradient(45deg, #667eea, #764ba2); color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 10px; font-weight: 600; flex: 1;">
                                🔍 Filter
                            </button>
                            <a href="{{ route('staff.bookings.index') }}" style="background: #6b7280; color: white; border: none; padding: 0.75rem 1rem; border-radius: 10px; text-decoration: none; display: flex; align-items: center;">
                                🔄
                            </a>
                        </div>
                    </div>
        ...
        @if(session('warning'))
            <div style="background: linear-gradient(45deg, #f59e0b, #d97706); color: white; padding: 1rem 1.5rem; border-radius: 15px; margin-bottom: 2rem; font-weight: 600; box-shadow: 0 10px 25px rgba(245, 158, 11, 0.3);">
                ⚠️ {{ session('warning') }}
            </div>
        @endif
        ...
        <div style="background: rgba(255,255,255,0.95); backdrop-filter: blur(15px); border-radius: 20px; overflow: hidden; box-shadow: 0 20px 40px rgba(0,0,0,0.1);">
            ...
            <div style="padding: 1.5rem; border-bottom: 1px solid #e5e7eb; background: #f9fafb;">
                <form id="bulkActionForm" method="POST" action="{{ route('staff.bookings.bulk-approve') }}">
                    @csrf
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted py-4">
                                    📭 No bookings found for your branch.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
<script>
    ...
    function approveBooking(bookingId) {
        ...
        form.action = `/staff/bookings/${bookingId}/approve`;
